...
export default function Quiz() {
  ...
  {questions.map((q, index) => (
    <div
      ...
      } ${questions.length % 3 === 1 && index === questions.length - 1 ? "col-span-3" : ""}`}
  ...
  {/* Main Quiz Area (Move it OUTSIDE the sidebar div) */}
  <div className="w-3/4 flex flex-col items-center p-6 mt-9">
    {/* Question */}
    <div className="bg-slate-200 w-3/5 text-lg font-semibold p-6 rounded-lg shadow-md w-3/5">
      ...
      {questions[currentQuestionIndex].options.map((option, index) => (
        <button
          ...
          className={`w-full text-black text-lg py-4 mt-4 rounded-lg font-medium transition-colors ${
            selectedOption === option ? "bg-green-500" : "bg-slate-400 hover:bg-stone-500"
          }`}
  ...
  <div className="w-3/5 flex justify-between border-3 border-red-800 ">
  <button className="border-2 border-white text-white rounded-lg px-14 py-4 mt-20 hover:bg-white hover:text-black transition duration-300" onClick={handleNext}>
  Prev
</button>

{currentQuestionIndex === questions.length - 1 ? (
  <button className="border-2 border-white text-white rounded-lg px-14 py-4 mt-20  hover:border-5  transition duration-300" onClick={handleSubmit}>
    Submit
  </button>
) : (
  <button className="border-2 border-white text-white rounded-lg px-14 py-4 mt-20 hover:bg-white hover:text-black transition duration-300" onClick={handleNext}>
