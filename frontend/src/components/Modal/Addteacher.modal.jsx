...
const AddTeacher = ({ closeModalAddTeacher }) => {
  useEffect(() => {
  ...
  return ReactDOM.createPortal(
    <div className="fixed top-0 left-0 w-screen h-screen bg-gray-800 bg-opacity-50 flex justify-center items-center z-50" onClick={closeModalAddTeacher}>
      <div className="bg-gray-900 text-white p-6 rounded-lg max-w-lg w-full sm:w-[500px] z-50 flex flex-col text-center shadow-lg hover:border-2 hover:border-white hover:shadow-2xl hover:shadow-gray-500 transition-all" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="bg-blue-500 text-white py-2 px-4 rounded-lg mt-4 hover:bg-blue-700 transition-colors" onClick={closeModalAddTeacher}>Accept It</button>
      </div>
    </div>,
